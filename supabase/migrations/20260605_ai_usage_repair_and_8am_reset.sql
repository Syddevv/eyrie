...
begin
  update public.ai_usage
    set message_count = case
          when last_reset <> v_current_cycle or limit_reset_at is null or limit_reset_at <= now()
            then 0
          else message_count
        end,
        reserved_count = case
...
begin
  ...
  update public.ai_usage
    set message_count = case
          when last_reset <> v_current_cycle or limit_reset_at is null or limit_reset_at <= now()
            then case when p_increment then 1 else 0 end
          else message_count + case when p_increment then 1 else 0 end
        end,
        reserved_count = case
          when last_reset <> v_current_cycle or limit_reset_at is null or limit_reset_at <= now()
            then 0
          else greatest(reserved_count - 1, 0)
        end,
        last_reset = case
          when last_reset <> v_current_cycle or limit_reset_at is null or limit_reset_at <= now()
            then v_current_cycle
