...
import { transactionRecords, type TransactionRecord } from '@/constants/transactions';
...
export default function TransactionsScreen() {
  ...
  const [calendarMonth, setCalendarMonth] = useState(selectedDate ?? new Date(2026, 4, 1));
  ...
  const filteredSections = useMemo(() => {
    ...
    const filteredTransactions = transactionRecords.filter((transaction) => {
      ...
      const matchesDate =
        !selectedDate || isSameDay(parseDateKey(transaction.dateKey), selectedDate);

      return matchesQuery && matchesType && matchesDate;
    });

    return Array.from(
      filteredTransactions.reduce((map, transaction) => {
  ...
  }, [searchQuery, selectedDate, typeFilter]);
