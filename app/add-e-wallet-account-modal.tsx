...
export default function AddEWalletAccountModal() {
  ...
  const returnTo = Array.isArray(params.returnTo) ? params.returnTo[0] : params.returnTo || '/add-e-wallet-method-modal';
      ...
      <View style={[styles.overlay, ui.overlay]}>
        ...
        <View
          ...
          <Text style={[styles.note, ui.note]}>
            For security purposes, phone number is optional because your e-wallet will not be directly connected to the app.
          </Text>

          <View style={[styles.walletCard, ui.walletCard]}>
            <View style={[styles.logoBubble, { backgroundColor: selectedWallet.color }]}>
              <Text style={styles.logoText}>{selectedWallet.code}</Text>
            </View>
            <View>
              <Text style={[styles.walletName, ui.walletTitle]}>{selectedWallet.name}</Text>
              <Text style={[styles.walletLabel, ui.walletSubtitle]}>{selectedWallet.label}</Text>
            </View>
          </View>
          ...
          <View style={styles.section}>
            <Text style={[styles.label, ui.label]}>Phone Number (Optional)</Text>
            <View style={[styles.fieldSurface, ui.fieldSurface]}>
              <TextInput
                value={phoneNumber}
